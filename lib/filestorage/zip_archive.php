...
class zip_archive extends file_archive {
    ...
    public function open($archivepathname, $mode=file_archive::CREATE, $encoding='') {
        ...
        if (empty($encoding)) {
            ...
            // if clients is Windows and zipfile is not mbz, use localewincharset as default charset
            // otherwise, use utf-8
            if(!empty($localewincharset) and strtolower(end(explode(".", $archivepathname))) != 'mbz' and check_browser_operating_system('Windows')) {
